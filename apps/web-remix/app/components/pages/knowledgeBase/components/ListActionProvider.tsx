...
export const FloatingListActions = ({ onDelete }: FloatingListActionsProps) => {
  ...
  return (
    <div
      className={cn(
        '!p-2 fixed z-[5] bg-white bottom-2 left-1/2 -translate-x-1/2 border border-input w-full max-w-[400px] rounded-lg flex gap-2 items-center justify-between',
        {
...
export const FloatingListCheckbox = ({ itemId }: FloatingListCheckboxProps) => {
  ...
  return (
    <label
      ...
      className={cn(
        'absolute top-0 left-0 -translate-x-1/3 -translate-y-1/3 z-[1] bg-white w-7 h-7 flex justify-center items-center rounded-md border border-input transition opacity-0 pointer-events-none group-hover:opacity-100 group-hover:pointer-events-auto',
        {
          'opacity-100 pointer-events-auto': isChecked,
        },
      )}
    >
      <CheckboxInput
