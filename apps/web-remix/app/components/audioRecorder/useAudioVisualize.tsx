...
export const useAudioVisualizeCircle = (
  canvas: RefObject<HTMLCanvasElement>,
  args?: Partial<OptionsType>,
) => {
  const { barHeightFactor, barPositionFactor, fftSize, onError, ...rest } = {
  ...
  const drawStaticCircle = useCallback(() => {
    const canvasContext = canvas.current?.getContext('2d');
    if (!canvas.current || !canvasContext) return;
    ...
    canvasContext.fillStyle = BASE_COLOR;
    canvasContext.beginPath();
    canvasContext.arc(centerX, centerY, BASE_RADIUS, 0, Math.PI * 2);
    canvasContext.fill();
  }, [handleClearCanvas]);
  ...
  const drawAudio = useCallback(() => {
    ...
    function draw() {
      ...
      const maxGrowth = canvas.current.width - BASE_RADIUS;
      const radius = BASE_RADIUS + (averageFrequency / 255) * maxGrowth;
