defmodule Buildel.LangChain.ChatModels.ChatGoogleAI do
  @moduledoc """
  ...
  @primary_key false
  embedded_schema do
    field :endpoint, :string, default: @default_base_url
  ...
  **NOTE:** This function *can* be used directly, but the primary interface
  ...
  def do_process_response(
        %{"content" => %{"parts" => _parts, "role" => _role} = content},
        message_type
      ) do
    do_process_response(
      %{
  ...
  def do_process_response(other, other2) do
    Logger.error("Trying to process an unexpected response. #{inspect(other)} #{inspect(other2)}")
    {:error, "Unexpected response"}
  end
